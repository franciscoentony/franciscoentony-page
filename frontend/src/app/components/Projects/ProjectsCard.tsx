        ...
        <div className='w-full h-full rounded-3xl bg-radial-[at_25%_25%] from-stone-800 overflow-hidden to-stone-950 to-100% p-5 flex flex-col justify-end items-center'>
            ...
            <div className="h-[55%] flex flex-col gap-3 justify-between">
                <div className="flex flex-col gap-5">
                    <p className='text-[15pt] w-full text-start'>{title}</p>
                    <span className='text-stone-400'>{description}</span>
                    <ul className='flex flex-wrap gap-x-3 gap-y-3 w-full h-auto'>
                        {technologies.map((technologie, index) => (
                            <li key={index} className='bg-emerald-900 flex cursor-default justify-center items-center h-7 px-4 rounded-full text-[10pt] hover:bg-emerald-700 duration-200 ease-in-out border border-transparent hover:border-emerald-300'>
                                {technologie.name}
                            </li>
                        ))}
                    </ul>
                ...
                <div className="w-full flex justify-start gap-5">
                    <Link href={'https://github.com'} className='flex gap-1 items-center hover:text-emerald-400 relative duration-300 bottom-0 ease-in-out hover:bottom-1'>
